...
pub struct CfgEnv<SPEC = SpecId> {
    ...
    /// Disables that ETH priority fees are transferred to the beneficiary
    /// of the block
    pub disable_priority_fee_transfer: bool,
}
...
impl<SPEC> CfgEnv<SPEC> {
    ...
    pub fn new_with_spec(spec: SPEC) -> Self {
        Self {
            ...
            disable_priority_fee_transfer: false,
        }
    }
...
impl<SPEC: Into<SpecId> + Copy> Cfg for CfgEnv<SPEC> {
    ...
    fn tx_gas_limit_cap(&self) -> u64 {
        self.tx_gas_limit_cap.unwrap_or(if self.spec.into().is_enabled_in(SpecId::OSAKA) {
    ...
    fn max_code_size(&self) -> usize {
        self.limit_contract_code_size.unwrap_or(eip170::MAX_CODE_SIZE)
    }
    ...
    fn is_priority_fee_transfer_disabled(&self) -> bool {
        self.disable_priority_fee_transfer
    }
}
